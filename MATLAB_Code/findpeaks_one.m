function [Ypk,Xpk,Wpk,Ppk] = findpeaks_one(Yin,min_height,p_distance,p_promi)
...
%  [y,yIsRow,x,xIsRow,Ph,Pp,Wmin,Wmax,Pd,Th,NpOut,Str,Ann,Ref] = parse_inputs(Yin,varargin)
%min_height,p_distance,p_promi)
yIsRow = isrow(Yin);
y = Yin(:);
  x = (1:numel(y)).';
xIsRow=yIsRow;
    minH = min_height;%p.Results.MinPeakHeight;
    minP = p_promi;%p.Results.MinPeakProminence;
    minW = 0;%p.Results.MinPeakWidth;
    maxW = Inf;%p.Results.MaxPeakWidth;
    minD = p_distance;%p.Results.MinPeakDistance;
    minT = 0;%.Results.Threshold;
    maxN =[];% p.Results.NPeaks;
    sortDir ='none';% p.Results.SortStr;
    annotate ='peaks';% p.Results.Annotate;
    refW ='halfprom';% p.Results.WidthReference;
...
% if coder.target('MATLAB')  %xiaolong
%     try %#ok<EMTC>
